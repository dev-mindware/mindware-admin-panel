...
import type { File as MyFile } from "@workspace/types";
    ...
    <div className="w-full">
          ...
          <div className="flex flex-col items-center gap-4 text-center">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Apenas arquivos de imagem (máx. {(maxSize / 1024 / 1024).toFixed(1)}
                MB)
              </p>
            </div>
          </div>
      ...
      {error && (
        <div className="mt-2 flex items-center gap-1.5 text-destructive">
          ...
          <p className="text-xs">Por favor, selecione um arquivo válido</p>
        </div>
      )}
    </div>
  );
...
export function PhotoUpload<T extends FieldValues>({
  label,
  name,
  info,
  control,
